...
PATTERN_MATCHING_DISTANCE_MODE // L1 or LSup
CuriePME_getDistanceMode(void)
{
	return (GCR_DIST & regRead16(GCR)) ? LSUP_Distance : L1_Distance;
}

void
CuriePME_setDistanceMode(PATTERN_MATCHING_DISTANCE_MODE mode) // L1 or LSup
{
...
uint16_t
CuriePME_getCommittedCount( void )
{
	return (getFORGET_NCOUNT() & 0xff );
}

PATTERN_MATCHING_CLASSIFICATION_MODE
CuriePME_getClassifierMode( void ) // RBF or KNN
{
...
void
CuriePME_setClassifierMode(PATTERN_MATCHING_CLASSIFICATION_MODE mode )
{
